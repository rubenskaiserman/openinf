...
[^4]: No texto do pdf é falado que é necessário no minimo as três coisas para realizar um algoritmo. Mas isso não é exatamente verdade dependendo da interpretação. Normalmente existem as três etapas, inclusive funções são baseadas nisso, mas existem inclusive funções sem parâmetro, e sem retorno. Então não é muito coerente dizer que essas três coisas são obrigatórias, mas também não é errado dizer isso.
